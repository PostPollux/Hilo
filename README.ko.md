...
![설정 → Hilo — Color palette 드롭다운, 테마 연동 8색 (슬러그/hex 잠금), Highlight style 드롭다운, 각 스타일 실시간 프리뷰](assets/screenshots/demo-03-settings.png)

- **Color palette 드롭다운** — 두 가지 색상 카탈로그 중 선택:
  - **Builtin (Theme)** — 옵시디언 테마 CSS 변수(`--color-red` ~ `--color-pink`)에 연동된 8색. 테마를 바꾸면 하이라이트 색도 즉시 따라옴. 슬러그/hex는 잠금(테마가 소유), 사용자 색을 위에 얹어 확장은 가능.
  - **Custom** — 3색 파스텔(yellow / red / green)로 시작, 전부 편집 가능.
- 두 팔레트가 공존. 한 팔레트에서 만든 하이라이트가 다른 팔레트로 스위치해도 유지됨 — 색상 맵을 병합하고 슬러그 충돌 시 활성 팔레트가 우선.
- **색상 row별 조작** — 활성화 토글, 화살표로 정렬, 슬러그·hex 편집, 단축키 할당(⌨ 버튼으로 Hotkeys 페이지 바로 이동), 삭제.
- 비활성화된 색상은 메뉴에서 숨겨지지만 기존 하이라이트는 계속 렌더됩니다.
...
Lowlight/Underlined 둘 다 `box-shadow`로 컬러 밴드를 렌더 — `text-decoration`이 Chromium spellcheck에 override되는 문제 회피.
...
### 색상 추가
1. 설정 → Hilo
2. 팔레트 선택 (Builtin은 테마 8색 위에 사용자 색 추가 가능, Custom은 전부 편집 가능)
3. **Add color** 버튼 클릭
4. 슬러그(예: `pink`) + hex(`#ffc0cb`) 입력
5. 즉시 모든 뷰에 반영됩니다.
...
- 슬러그가 설정 카탈로그에 없으면 옵시디언 네이티브 `==text==` 노란색으로 fallback됩니다 — 깨지지 않고 그대로 보이되 색만 옵시디언 기본값.
...
Hilo는 [Scaff](https://github.com/opellen/Scaff)로 개발되었습니다 — 가볍고 마크다운 기반인 AI 개발 harness. 모든 단계(페이즈별 구축, 마켓 리뷰 대응, 데모 자산 제작)를 CLI/훅/MCP 없이 `CONTEXT.md` / `GOAL.md` / `ROADMAP.md` 평문 파일로 추적했습니다.
